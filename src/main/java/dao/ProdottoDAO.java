...
*/
	...
	public boolean modifica(Prodotto prodotto) {
		String sql = "UPDATE prodotti SET " + "nome = ?, " + "gioco = ?, " + "categoria = ?, " + "rarita = ?, "
				+ "condizione = ?, " + "prezzo = ?, " + "quantita = ?, " + "immagine = ?, " + "descrizione = ?, "
				+ "stato = ? " + "WHERE id = ?";
...
*/
	public ArrayList<Prodotto> cercaProdotti(String ricerca, String gioco, String categoria) {
		...
		String sql = "SELECT * FROM prodotti " + "WHERE stato = 'DISPONIBILE' " + "AND quantita > 0 "
				+ "AND (? IS NULL OR nome LIKE ?) " + "AND (? IS NULL OR gioco = ?) "
				+ "AND (? IS NULL OR categoria = ?) " + "ORDER BY nome";
